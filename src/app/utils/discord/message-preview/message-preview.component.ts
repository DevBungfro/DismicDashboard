...
export class MessagePreviewComponent {
  @Input() guild = { name: 'another hidden message', memberCount: 420 };
  @Input() eventVariables = true;

  @Input() content = 'hidden message gj on finding this';
  @Input() author = {
    username: 'Dismic',
  ...
  get processed() {
    ...
    return (this.eventVariables) ? toHTML(textEmoji(this.content
      ...
      .replace(/\[INSTIGATOR\]/, '@Dismic#9277')
